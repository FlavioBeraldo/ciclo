...
function InlineLeadForm({ serviceName }: { serviceName: string }) {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      await fetch('/api/pipedrive', {
        ...
        body: JSON.stringify({ ...data, source: `Site/WhatsApp – Serviço: ${serviceName}` }),
      })
    } catch {
  ...
  return (
    <section
      id="contato-servico"
      className="relative overflow-hidden rounded-3xl border border-[#A100FF]/20 bg-gradient-to-br from-[#1a0030]/80 via-[#0d0020]/80 to-[#050505]/80 p-8 lg:p-12"
    >
      {/* Background glow */}
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] rounded-full bg-[#A100FF]/12 blur-[100px]" />
      </div>
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        >
          ...
          <p className="text-[#A1A1AA] text-sm sm:text-base">
            Agende uma conversa estratégica sem compromisso e descubra o que a Ciclo pode fazer
            pela sua marca.
          </p>
        </motion.div>
            ...
            {submitted ? (
              ...
              >
                ...
                <p className="text-[#A1A1AA] max-w-md">
                  Recebemos suas informações. Em breve nosso time entrará em contato para agendar
                  sua conversa estratégica.
                </p>
              </motion.div>
            ) : (
              <motion.form
              ...
              >
                ...
                <div className="sm:col-span-2">
                  <p className="text-xs text-[#A1A1AA] mb-4">
                    Quanto mais contexto você trouxer, melhor conseguimos direcionar a conversa
                    estratégica.
                  </p>
                  <Button
                    type="submit"
...
export function ServicosInteractive({ servico }: { servico: Servico }) {
  return (
    <>
      ...
      {/* CTA form */}
      <InlineLeadForm serviceName={servico.title} />
    </>
  )
}
